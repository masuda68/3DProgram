...
void Application::Update()
{
	...
	//全ゲームオブジェクト更新
	{
		for (std::shared_ptr<KdGameObject>gameOgj : m_GameObjList)
		{
			//地球の座標もらう
			if (gameOgj->GetObjeTyoe() == KdGameObject::ObjectType::moon)
			{
				for (std::shared_ptr<KdGameObject>_earth : m_GameObjList)
				{
					if (_earth->GetObjeTyoe() == KdGameObject::ObjectType::earth)
					{
						gameOgj->SetPos(_earth->GetPos());
						gameOgj->Update(); 
						continue;
					}
				}
			...
			////地球に太陽を座標を渡す
			//if (gameOgj->GetObjeTyoe() == KdGameObject::ObjectType::earth)
			//{
			//	for (std::shared_ptr<KdGameObject>_sun : m_GameObjList)
			//	{
			//		if (_sun->GetObjeTyoe() == KdGameObject::ObjectType::sun)
			//		{
			//			gameOgj->SetPos(_sun->GetPos());
			//			gameOgj->Update();
			//			continue;
			//		}
			//	}
