...
public:
	...
	{
		if (m_cb0_Obj.Work().EnbleOutLineDraw != static_cast<int>(enableOutLineDraw))
		{
			m_cb0_Obj.Work().EnbleOutLineDraw = enableOutLineDraw;
			m_dirtyCBObj = true;
		}
	}
	...
	void SetIBLTexture(KdTexture& ibTex)
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(12, 1, ibTex.WorkSRViewAddress());
	}
...
private:
	...
	void ResetCBObject();
	// アウトライン描画設定フラグの取得
	bool FetEnableOutLineDraw() { return m_cb0_Obj.Work().EnbleOutLineDraw; }
	...
	bool							m_dirtyCBObj = false; //定数バッファのオブジェクトに変更があったどうか
};
